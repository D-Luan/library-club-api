CREATE TABLE Readers (
    ...
    CONSTRAINT CK_Readers_Status CHECK (Status IN (N'Active', N'Inactive'))
);
...
CREATE TABLE ReadingClubs (
    ...
    CONSTRAINT CK_ReadingClubs_Status CHECK (Status IN (N'Active', N'Inactive', N'Archived'))
);
...
CREATE TABLE ClubSubscriptions (
    ...
    CONSTRAINT CK_ClubSubscriptions_Status
        CHECK (Status IN (N'Active', N'Canceled')),

    CONSTRAINT CK_ClubSubscriptions_CanceledAt
        CHECK (
            (Status = N'Active' AND CanceledAt IS NULL)
            OR
            (Status = N'Canceled' AND CanceledAt IS NOT NULL)
        )
);

CREATE INDEX IX_Readers_CreatedAt
ON Readers (CreatedAt DESC, Id DESC);

CREATE INDEX IX_ReadingClubs_CreatedAt
ON ReadingClubs (CreatedAt DESC, Id DESC);

CREATE INDEX IX_ClubSubscriptions_ReaderId_CreatedAt
ON ClubSubscriptions (ReaderId, CreatedAt DESC, Id DESC);

CREATE INDEX IX_ClubSubscriptions_ReadingClubId_CreatedAt
ON ClubSubscriptions (ReadingClubId, CreatedAt DESC, Id DESC);
